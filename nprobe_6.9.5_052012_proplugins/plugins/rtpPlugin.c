...
static V9V10TemplateElementId rtpPlugin_template[] = {
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID,   STATIC_FIELD_LEN, 4,  numeric_format, dump_as_uint, "RTP_FIRST_SSRC",       "", "First flow RTP Sync Source ID" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+1, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_uint, "RTP_FIRST_TS",         "", "First flow RTP timestamp" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+2, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_uint, "RTP_LAST_SSRC",        "", "Last flow RTP Sync Source ID" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+3, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_uint, "RTP_LAST_TS",          "", "Last flow RTP timestamp" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+4, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_uint, "RTP_IN_JITTER",        "", "RTP Jitter (ms * 1000)" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+5, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_uint, "RTP_OUT_JITTER",       "", "RTP Jitter (ms * 1000)" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+6, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_formatted_uint, "RTP_IN_PKT_LOST",      "", "Packet lost in stream" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+7, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_formatted_uint, "RTP_OUT_PKT_LOST",     "", "Packet lost in stream" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+8, STATIC_FIELD_LEN, 1,  numeric_format, dump_as_uint, "RTP_OUT_PAYLOAD_TYPE", "", "RTP payload type" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+9, STATIC_FIELD_LEN, 4,  numeric_format, dump_as_uint, "RTP_IN_MAX_DELTA",     "", "Max delta (ms*100) between consecutive pkts" },
  { 0, BOTH_IPV4_IPV6, FLOW_TEMPLATE, LONG_SNAPLEN, NTOP_ENTERPRISE_ID, BASE_ID+10, STATIC_FIELD_LEN, 4, numeric_format, dump_as_uint, "RTP_OUT_MAX_DELTA",    "", "Max delta (ms*100) between consecutive pkts" },
...
struct rtp_plugin_info {
  ...
  u_int16_t src2dstInitialSeqNumber, dst2srcInitialSeqNumber;
  u_int16_t src2dstLastSeqNumber, dst2srcLastSeqNumber;
...
static PluginEntryPoint rtpPlugin; /* Forward */
...
static void rtpPlugin_packet(u_char new_bucket,
  ...
  if(new_bucket /* This bucket has been created recently */) {
    ...
    if(readOnlyGlobals.enable_debug)
      traceEvent(TRACE_INFO, "==> Called rtpPlugin_packet(%d)[proto=%d][payload(0)=%02X][payload_type=%02X]",
		 new_bucket, bkt->core.tuple.proto, payload[0] & 0xFF, payload_type);

    /* Check whether this is an RTP flow */
    if((bkt->core.tuple.proto == 17)
	...
	return; /* Not enough memory */
      ...
      pluginData = info->pluginData = (struct rtp_plugin_info*)malloc(sizeof(struct rtp_plugin_info));
	...
	/* Reset fields */
	memset(pluginData, 0, sizeof(struct rtp_plugin_info));

	/* Defaults */
	((struct rtp_plugin_info*)pluginData)->payloadType = -1; /* Unknown */
  ...
  */
  if(pluginData != NULL) {
    ...
    */
	...
	info->src2dstInitialSeqNumber = sequence_num;
      } else {
	struct timeval tv;
	u_int32_t delta;
	...
	if(delta > info->src2dstMaxDelta) info->src2dstMaxDelta = delta;
      ...
      info->src2dstLastSeqNumber = sequence_num;
	...
	info->dst2srcInitialSeqNumber = sequence_num;
      } else {
	struct timeval tv;
        u_int32_t delta;
	...
	if(delta > info->dst2srcMaxDelta) info->dst2srcMaxDelta = delta;
      ...
      info->dst2srcLastSeqNumber = sequence_num;
...
static V9V10TemplateElementId* rtpPlugin_get_template(char* template_name) {
  ...
  for(i=0; rtpPlugin_template[i].templateElementId != 0; i++) {
    if(!strcmp(template_name, rtpPlugin_template[i].netflowElementName)) {
...
static void sanitizeInfo(FlowHashBucket *bkt, struct rtp_plugin_info *info) {
  if(bkt->core.tuple.flowCounters.pktSent < 64)
    info->src2dstJitter = 0;

  if(bkt->core.tuple.flowCounters.pktRcvd < 64)
    info->dst2srcJitter = 0;

  if(info->src2dstJitter == 0) info->src2dstJitter = (double)-1/(double)1000000;
  if(info->dst2srcJitter == 0) info->dst2srcJitter = (double)-1/(double)1000000;
}
...
static int rtpPlugin_export(void *pluginData, V9V10TemplateElementId *theTemplateElement,
  ...
  for(i=0; rtpPlugin_template[i].templateElementId != 0; i++) {
	...
	sanitizeInfo(bkt,info);

	switch(rtpPlugin_template[i].templateElementId) {
	case BASE_ID:
	  copyInt32(direction == src2dst_direction ? info->src2dstInitialSeqNumber : info->dst2srcInitialSeqNumber,
		    outBuffer, outBufferBegin, outBufferMax);
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO, "RTP_FIRST_SSRC: %u",
						     direction == src2dst_direction ? info->src2dstInitialSeqNumber : info->dst2srcInitialSeqNumber);

	  break;
	case BASE_ID+1:
	  copyInt32(direction == dst2src_direction ? info->src2dstInitialTimestamp : info->dst2srcInitialTimestamp,
		    outBuffer, outBufferBegin, outBufferMax);
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO, "RTP_FIRST_TS: %u",
						     direction == dst2src_direction ? info->src2dstInitialTimestamp : info->dst2srcInitialTimestamp);

	  break;
	case BASE_ID+2:
	  copyInt32(direction == src2dst_direction ? info->src2dstLastSeqNumber : info->dst2srcLastSeqNumber,
		    outBuffer, outBufferBegin, outBufferMax);
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO, "RTP_FIRST_SSRC: %u",
						     direction == src2dst_direction ? info->src2dstLastSeqNumber : info->dst2srcLastSeqNumber);

	  break;
	case BASE_ID+3:
	  copyInt32(direction == src2dst_direction ? info->src2dstPrevTimestamp : info->dst2srcPrevTimestamp,
		    outBuffer, outBufferBegin, outBufferMax);
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO, "RTP_FIRST_TS: %u",
						     direction == src2dst_direction ? info->src2dstPrevTimestamp : info->dst2srcPrevTimestamp);
	...
	case BASE_ID+4:
	  ...
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO, "RTP_IN_JITTER: %.2f ms",
						     (jitter == (u_int32_t)-1) ? -1 : (float)jitter/1000);
	...
	case BASE_ID+5:
	  ...
	  copyInt32(jitter, outBuffer, outBufferBegin, outBufferMax);
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO, "RTP_OUT_JITTER: %.2f ms", 
						     (jitter == (u_int32_t)-1) ? -1 : (float)jitter/1000);

	  break;
	case BASE_ID+6:
	  if(bkt->core.tuple.flowCounters.pktSent > 0) {
	    u_int32_t tot;

	    tot = pktLost = info->src2dstLastSeqNumber+1 - info->src2dstInitialSeqNumber;
	    pktLost -= bkt->core.tuple.flowCounters.pktSent; if(pktLost < 0) pktLost = 0;
	    pktLoss = (float)(pktLost*10000)/(float)tot;
	  } else {
	    pktLost = 0; pktLoss= 0;
	  }
	  ...
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO,
						     "RTP_IN_PKT_LOST: %.1f%% [%u pkts] [%u <-> %u]",
						     pktLoss/100, pktLost,
						     info->src2dstInitialSeqNumber, info->src2dstLastSeqNumber);
	  break;
	  
	case BASE_ID+7:
	  if(bkt->core.tuple.flowCounters.pktRcvd > 0) {
	    u_int32_t tot;

	    tot = pktLost = info->dst2srcLastSeqNumber+1 - info->dst2srcInitialSeqNumber;
	    pktLost -= bkt->core.tuple.flowCounters.pktRcvd; if(pktLost < 0) pktLost = 0;
	    pktLoss = (float)(pktLost*10000)/(float)tot;
	  } else {
	    pktLost = 0; pktLoss= 0;
	  }
	  ...
	  copyInt32((u_int32_t)pktLost, outBuffer, outBufferBegin, outBufferMax);
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) traceEvent(TRACE_INFO, "RTP_OUT_PKT_LOST: %.1f%% [%u pkts]  [%u <-> %u]",
						     pktLoss/100, pktLost,
						     info->dst2srcInitialSeqNumber, info->dst2srcLastSeqNumber);
	  break;
	case BASE_ID+8:
	  copyInt8(info->payloadType, outBuffer, outBufferBegin, outBufferMax);
	  if(readOnlyGlobals.enable_debug)
	    if(readOnlyGlobals.traceMode) 
	      traceEvent(TRACE_INFO, "RTP_PAYLOAD_TYPE: %u", info->payloadType);
...
static int rtpPlugin_print(void *pluginData, V9V10TemplateElementId *theTemplateElement,
  ...
  for(i=0; rtpPlugin_template[i].templateElementId != 0; i++) {
	...
	case BASE_ID+3:
	  ...
	  break;
	case BASE_ID+4:
	  snprintf(&line_buffer[strlen(line_buffer)], (line_buffer_len-strlen(line_buffer)), "%ld",
		   (long)(info->src2dstJitter*1000000));
	  break;
	case BASE_ID+5:
	  snprintf(&line_buffer[strlen(line_buffer)], (line_buffer_len-strlen(line_buffer)), "%ld",
		   (long)(info->dst2srcJitter*1000000));
	  break;
	case BASE_ID+6:
	case BASE_ID+7:
	  if(bkt->core.tuple.flowCounters.pktSent > 0) {
	    ...
	    pktLost -= bkt->core.tuple.flowCounters.pktSent; if(pktLost < 0) pktLost = 0;
	    pktLoss = (float)(pktLost*10000)/(float)bkt->core.tuple.flowCounters.pktSent;
	  } else {
	    pktLost = 0; pktLoss= 0;
	  }
...
/* Plugin entrypoint */
static PluginEntryPoint rtpPlugin = {
  NPROBE_REVISION,
  "RTP",
  "0.2",
...
PluginEntryPoint* rtpPluginEntryFctn(void)
#else
