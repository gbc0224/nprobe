...
static PluginEntryPoint dbPlugin; /* Forward */
...
void dbPlugin_init() {
  ...
  if((readOnlyGlobals.argc == 2) && (readOnlyGlobals.argv[1][0] != '-')) {
    FILE * fd;
    char   line[256];

    fd = fopen(readOnlyGlobals.argv[1], "r");
    if(fd == NULL) {
      traceEvent(TRACE_ERROR, "Unable to read config. file %s", readOnlyGlobals.argv[1]);
      fclose(fd);
      return;
    }
	...
	skip_db_creation = 1;
		...
		&& ((line[strlen(MYSQL_OPT)] == ' ') || (line[strlen(MYSQL_OPT)] == '='))) {
    ...
    for(i=0; i<readOnlyGlobals.argc; i++)
      ...
      if(strncmp(readOnlyGlobals.argv[i], MYSQL_SKIP_DB_CREATION, strlen(MYSQL_SKIP_DB_CREATION)) == 0) {
	skip_db_creation = 1;
      } else if((strncmp(readOnlyGlobals.argv[i], MYSQL_OPT, strlen(MYSQL_OPT)) == 0) 
		&& ((readOnlyGlobals.argv[i][strlen(MYSQL_OPT)] == ' ') 
		    || (readOnlyGlobals.argv[i][strlen(MYSQL_OPT)] == '='))) {
	char *mysql_arg = readOnlyGlobals.argv[i+1];
	int sz;

	if(readOnlyGlobals.argv[i][strlen(MYSQL_OPT)] == '=') {
	  mysql_arg = &readOnlyGlobals.argv[i][strlen(MYSQL_OPT)+1];
       } else
	  mysql_arg = readOnlyGlobals.argv[i+1];
...
static void dbPlugin_packet(u_char new_bucket,
...
static V9V10TemplateElementId* dbPlugin_get_template(char* template_name) {
  ...
  for(i=0; dbPlugin_template[i].templateElementId != 0; i++) {
    if(!strcmp(template_name, dbPlugin_template[i].netflowElementName)) {
...
static int dbPlugin_export(void *pluginData, V9V10TemplateElementId *theTemplate, 
...
static int dbPlugin_print(void *pluginData, V9V10TemplateElementId *theTemplate, FlowDirection direction,
...
static void dbPlugin_help(void) {
  ...
  printf("  %s                           | Skip database schema creation\n", MYSQL_SKIP_DB_CREATION);
#endif
}
...
PluginEntryPoint* dbPluginEntryFctn(void)
#else
