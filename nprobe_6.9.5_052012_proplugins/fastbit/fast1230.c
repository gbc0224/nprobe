  ...
  for(i=0, fb_idx = 0; i<TEMPLATE_LIST_LEN; i++) {
	...
	if((elems[i]->templateElementLen <= 6)
	  ...
	  if(elems[i]->templateElementLen == 1) {
	    ...
	    int x = fb_idx-1;
	   // traceEvent(TRACE_NORMAL, "%s = %u",elems[i]->netflowElementName, ptr);
	   // traceEvent(TRACE_NORMAL, "%s = %u",elems[i]->netflowElementName,readWriteGlobals->fastbit.fb_element[fb_idx++]);
#ifdef FASTBIT_DEBUG
