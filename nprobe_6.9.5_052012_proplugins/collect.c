 ...
 *
 *       Copyright (C) 2007-12 Luca Deri <deri@ntop.org>
 *
 *                     http://www.ntop.org/
 *
...
inline u_int32_t getField3264to32(V9V10TemplateField *field, char *buffer) {
  u_int32_t val32;
  u_int64_t val64;
  ...
  case 8:
    ...
    val32 = (u_int32_t)ntohll(val64);
    val32 = htonl(val32); /* Re-endian */
    break;
  default:
	  ...
	  while((bufferLen >= (displ+stillToProcess)) && (!templateDone)) {
		...
		if(((template.fieldCount * 4) + sizeof(FlowSet) + 4 /* templateFlowSet + FlowsetLen */) >  header.flowsetLen) {
		  ...
		  if(bufferLen < (displ+stillToProcess)) {
		    traceEvent(TRACE_INFO, "Broken flow format (bad length)");
		    return;
		  }
		...
		if(readOnlyGlobals.enable_debug) {
		  /* if(cursor->templateInfo.isOptionTemplate) */ {
		    traceEvent(TRACE_NORMAL, ">>>>> Dissecting flow field "
			       "[optionTemplate=%d][displ=%d/%d][template=%d][fieldId=%d][fieldLen=%d][isPenField=%d][field=%d/%d] [%d...%d] [accum_len=%d] [%02X %02X %02X %02X]",
		...
		if(fields[fieldId].isPenField == 0) {
		  ...
		  */
		  ...
		  case 1: /* IN_BYTES Incoming flow bytes (src->dst) */
		    record.sentOctets = getField3264to32(&fields[fieldId], &buffer[displ]);
		    break;
		  case 2: /* IN_PKTS */
		    record.sentPkts = getField3264to32(&fields[fieldId], &buffer[displ]);
		    break;
		  case 4: /* PROT */
		  ...
		  case 23: /* OUT_BYTES Outgoing flow bytes (dst->src) */
		    record.rcvdOctets = getField3264to32(&fields[fieldId], &buffer[displ]);
		    break;
		  case 24: /* OUT_PKTS */
		    record.rcvdPkts = getField3264to32(&fields[fieldId], &buffer[displ]);
		    break;
